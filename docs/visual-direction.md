...
| HUD / timer | **A Classic** — navy rounded pills, gold numbers, gold circle pause, green-to-gold timer bar, Fredoka/Nunito type. Labels: Level, Score, Best. |
| Menu | **Poster** — stacked Titan One wordmark in the sky, manila how-to tag, toy navy **Game options** + gold **Start Climb**, adventurer on the grass. |
| End overlay | **Report card** — cream paper sheet, score cheer title, giant score, NEW RECORD pill, Climb again, Menu |
| Title type | Titan One wordmark in the sky |
| Player | **Adventurer** from `public/assets/adventurer.png` (80×110 cells). Idle `0,0` / `1,8`. Jump duck `1,0` → air `0,4` → kick `2,2`. Nearest-neighbor scale. **Drop shadow is runtime, not baked:** navy `#16324F` at 35% opacity, offset 2px right / 4px down (at 1× display). |
