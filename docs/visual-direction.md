...
| Player | **Adventurer** from `reference assets/adventurer_tilesheet.png` (80×110 cells). Idle `0,0` / `1,8`. Jump duck `1,0` → air `0,4` → kick `2,2`. **Drop shadow is part of the sprite:** navy `#16324F` at 35% opacity, offset 2px right and 4px down at 1× (scale with nearest-neighbor). Do not omit it in the atlas or in-engine. Foozle robot unused. |
...
Adventurer atlas from the tilesheet (idle + jump, transparent, integer scale) **with the navy drop shadow baked into every frame**. Sod platforms as Phaser graphics or a sprite. Pause/CTA as Phaser + type matching A Classic.
